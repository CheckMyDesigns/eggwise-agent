...
session context, so it never asks who you are. Four tabs:
  Home  - greeting, adherence + status cards, encouragement, quick actions
  Chat  - a real conversation with the Companion (suggested prompts, history); clinical
          questions hit the medical-safety guardrail and escalate to the care team
  Care  - set medication reminders, book follow-ups, and log a daily check-in
  Learn - clinic-approved answers to common logistics/prep questions
...
_PATIENT_CSS = r"""
  ...
  .cards{display:grid;grid-template-columns:repeat(auto-fill,minmax(210px,1fr));gap:14px;max-width:720px;margin-bottom:18px}
  ...
  .encourage{max-width:720px;background:linear-gradient(135deg,rgba(116,160,160,.12),rgba(255,215,0,.07));border:1px solid var(--line);border-radius:14px;padding:14px 16px;margin-bottom:16px;font-size:14px;color:var(--ink-soft)}
  .qa{display:flex;gap:10px;flex-wrap:wrap;max-width:720px}
  /* chat */
  .chatwrap{display:flex;flex-direction:column;min-height:calc(100dvh - 160px)}
  ...
  .msg.me{align-self:flex-end;background:linear-gradient(135deg,var(--teal),var(--teal-deep));color:#fff;border-bottom-right-radius:5px}
  ...
  @media (max-width:760px){ .chatin input{font-size:16px} .chatwrap{min-height:calc(100dvh - 200px)} }
"""
...
async function renderHome(){
  $('#main').innerHTML=`<div class="hero">Hi ${esc(firstName(PNAME))} &#128075;</div>
   ...
   <div id="ad"><div class="empty">Loading <span class="spin"></span></div></div>
   <div class="encourage" id="enc"></div>
   <div class="qa">
     <button class="btn teal" onclick="go('chat')">Ask EggWise</button>
     <button class="btn ghost" onclick="go('care')">Daily check-in</button>
     <button class="btn ghost" onclick="go('care')">Set a reminder</button>
     <button class="btn ghost" onclick="go('learn')">Learn</button>
   </div>`;
  const a=await api('/api/adherence?patient_id='+encodeURIComponent(PID));
  if(a.error||a.name===undefined){$('#ad').innerHTML='';$('#enc').innerHTML='';return;}
  const pct=a.days_reviewed?Math.round((a.days_reviewed-a.doses_missed)/a.days_reviewed*100):100;
  $('#ad').innerHTML=`<div class="cards">
    <div class="card"><div class="lab">Adherence (${a.days_reviewed}d)</div><div class="big ${a.on_track?'ok':'warn'}">${pct}%</div><span class="pill ${a.on_track?'ok':'warn'}">${a.on_track?'On track':a.doses_missed+' dose(s) missed'}</span></div>
  ...
  $('#enc').textContent=a.on_track?"You're doing great. Keeping a steady routine really helps. Want me to set a reminder so it stays easy?":"It's okay, missing a dose happens. Let's set a reminder together to make the next ones easier.";
}

/* CHAT */
function renderChat(){
  $('#main').innerHTML=`<div class="chatwrap">
    <div class="chips">${SUGGEST.map(s=>`<button class="chip2" data-q="${esc(s)}" onclick="ask(this.dataset.q)">${esc(s)}</button>`).join('')}</div>
    <div class="tx" id="tx"></div>
    <form class="chatin" id="chatForm"><input id="ci" placeholder="Message EggWise..." autocomplete="off"><button class="btn teal" type="submit">Send</button></form>
  </div>`;
  ...
  drawChat();
  $('#chatForm').addEventListener('submit',e=>{e.preventDefault();const v=$('#ci').value.trim();if(v){$('#ci').value='';ask(v);}});
  $('#ci').focus();
}
function drawChat(){const tx=$('#tx');if(!tx)return;tx.innerHTML=CHAT.map(m=>`<div class="msg ${m.role==='me'?'me':'bot'}">${m.html}</div>`).join('');tx.scrollTop=tx.scrollHeight;
  const w=tx.parentElement;if(w)w.scrollTop=w.scrollHeight;$('#main').scrollTop=$('#main').scrollHeight;}
async function ask(text){
  if($('#main').querySelector('.chatwrap')===null)go('chat');
  CHAT.push({role:'me',html:esc(text)});
  const idx=CHAT.push({role:'bot',html:'<span class="spin"></span>'})-1;drawChat();
  const r=await jpost('/api/agent',{message:text,audience:'patient',patient_id:PID,patient_name:PNAME});
  CHAT[idx].html=r.error?`<span class="muted">Companion unavailable here (${esc(r.error)}). Reminders and booking still work.</span>`:esc(r.text||'(no response)');
  drawChat();
}
...
function renderCare(){
  ...
  <div class="panel"><h3>Daily check-in</h3>
    <div class="field"><label>Did you take your medication today?</label><div class="seg"><button id="mYes" class="on" onclick="setMeds(true)">Yes</button><button id="mNo" onclick="setMeds(false)">Not yet</button></div></div>
    <div class="row2"><div class="field"><label>Mood (1 low - 5 great)</label><select id="cMood"><option>1</option><option>2</option><option selected>3</option><option>4</option><option>5</option></select></div>
    <div class="field"><label>Note (optional)</label><input id="cNote" placeholder="Anything you want to remember"></div></div>
    <button class="btn teal" onclick="doCheckin()">Log check-in</button><div id="cOut"></div></div>`;
}
let MEDS=true;
...
async function doReminder(){const r=await jpost('/api/reminder',{patient_id:PID,medication:$('#rMed').value,time:$('#rTime').value,days:$('#rDays').value});
  $('#rOut').innerHTML=r.error?`<div class="result">${esc(r.error)}</div>`:`<div class="result"><b>${esc(r.medication)}</b> · ${esc(r.frequency)}<br><br><a class="btn gold" href="${r.google_calendar_link}" target="_blank">Add to Google Calendar</a></div>`;}
async function doBook(){const r=await jpost('/api/book',{patient_id:PID,date:$('#bDate').value,time:$('#bTime').value});
  $('#bOut').innerHTML=r.error?`<div class="result">${esc(r.error)}</div>`:`<div class="result"><b>${esc(r.title||'Follow-up')}</b><br>${esc(r.start||'')}<br><br><a class="btn gold" href="${r.google_calendar_link}" target="_blank">Add to Google Calendar</a></div>`;}
async function doCheckin(){const r=await jpost('/api/checkin',{patient_id:PID,meds_taken:MEDS,mood:$('#cMood').value,note:$('#cNote').value});
  $('#cOut').innerHTML=`<div class="result">${esc(r.message||'Logged.')}</div>`;toast('Check-in logged');}

/* LEARN */
async function renderLearn(){
  $('#main').innerHTML=`<h1>Learn</h1><p class="sub">Clinic-approved answers to common questions. For anything about your specific care, just ask and I'll loop in your team.</p><div class="lcards" id="lc"><div class="empty">Loading <span class="spin"></span></div></div>`;
